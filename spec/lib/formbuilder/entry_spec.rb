...
def change_field_type(rf, new_field_type)
  rf.becomes!("Formbuilder::ResponseField#{new_field_type}".constantize)
  rf.reload
end
...
describe Formbuilder::Entry do
  ...
  describe 'callbacks' do
    ...
    describe 'calculate_responses_text' do
      it 'gets called when responses change and entry is saved' do
        ...
        entry.save_responses({ "#{form.response_fields.first.id}" => 'boo' }, form.response_fields)
        entry.save
      end
    end
  end
  ...
  describe '#value_present?' do
    it 'should be true if there is a value' do
      entry.value_present?(form.response_fields.first).should == false
      entry.responses[form.response_fields.first.id.to_s] = 'foo'
      entry.value_present?(form.response_fields.first).should == true
    end

    it 'should be true if there are no options for a radio/checkbox/dropdown/etc' do
      entry.value_present?(form.response_fields.first).should == false
      change_field_type(form.response_fields.first, 'Checkboxes')
      puts form.response_fields.first.inspect
      entry.value_present?(form.response_fields.first).should == true
    end

    it 'should be typerue if value is a hash and has at least one response' do
      form.response_fields.first.update_attributes(type: 'Formbuilder::ResponseFieldTime')
      entry.value_present?(form.response_fields.first).should == false
      entry.responses[form.response_fields.first.id.to_s] = { 'am_pm' => 'am' }.to_yaml
      entry.value_present?(form.response_fields.first).should == false
      entry.responses[form.response_fields.first.id.to_s] = { 'minutes' => '06' }.to_yaml
      entry.value_present?(form.response_fields.first).should == true
    end
  end
  ...
  #         first_response_field = Formbuilder::ResponseField.find(first_response_field.id)
